...
// it isn't set. Mirrors internal/store/dbtest_test.go and
// internal/indexer/dbtest_test.go — duplicated rather than exported
// because this package must only reach into store's public API.
...
func newTestSchema(t *testing.T) *pgxpool.Pool {
	...
	schemaName := "qoge_query_test_" + randomHex(t, 12)
...
// newTestQueryStore returns a fully-migrated, isolated schema with both a
// write internal/store.Store (to build canonical fixtures via ApplyBlock/
// RollbackTo — never ad-hoc SQL) and the read-only query.Store under test,
// sharing one pool.
func newTestQueryStore(t *testing.T) (*Store, *store.Store, *pgxpool.Pool) {
	t.Helper()
	ctx := context.Background()
	...
	return New(pool), store.New(pool), pool
}
