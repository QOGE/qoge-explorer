...
// testDatabaseURL/newTestSchema mirror internal/query/dbtest_test.go (and
// internal/store's, internal/indexer's) — duplicated per package on
// purpose so this package only ever reaches into internal/store's and
// internal/query's PUBLIC API, never their test internals.
...
func newTestSchema(t *testing.T) *pgxpool.Pool {
	...
	schemaName := "qoge_api_test_" + randomHex(t, 12)
...
// newTestServer returns a fully-migrated, isolated schema with a write
// internal/store.Store (to build canonical fixtures via ApplyBlock — never
...
// internal/mempool.Store (which needs the pool — there is no write mempool
// access through Server itself, mirroring internal/query's own read/write
// separation).
